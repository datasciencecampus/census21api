...
class Interface:
    ...
    def execute_api_call(self):
        ...
        if var_1 != "None" and var_2 != "None":
            data = self.api.query_api(
                self.residence_code, f"{var_1},{var_2}", self.region
            )
        elif var_1 == "None":
            data = self.api.query_api(
                self.residence_code, f"{var_2}", self.region
            )
        else:
            data = self.api.query_api(
                self.residence_code, f"{var_1}", self.region
            )
    ...
    def create_variable_selection(self):
        ...
        self.variable_drop_1 = self.drop_config(
            "Variable 1", self.variable_list, row=2, column=2
        )
        self.variable_drop_2 = self.drop_config(
            "Variable 2", self.variable_list, row=2, column=3
        )
        self.area_codes = self.list_areas()
        self.regional_drop = self.drop_config(
            "Regional Divisions", self.area_codes, row=2, column=4
        )
    ...
    def list_variables(self):
        ...
        self.variable_list = list(
            self.api.get_dims_by_pop_type(self.residence_code).values()
        )
    ...
    def list_areas(self):
        ...
        """
        area_list = list(
            self.api.get_areas_by_pop_type(self.residence_code).keys()
        )
    ...
    def start_selections(self):
        ...
        """
        self.residence_drop = self.drop_config_button(
            "Residence Selection",
            ("Usual Residence", "Household", "Household Reference Person"),
            row=2,
            column=1,
            command=self.retrieve_res_input,
        )
