...
class APIWrapper:
    ...
    def set_population_types_available(self) -> List[str]:
        ...
        possible_codes: Set[str] = set(
            [
                self.extract_pop_type_from_item(item)
                for item in self._current_data["items"]
            ]
        )
        ...
        for cd in possible_codes:
            ...
            if (
                isinstance(self._current_data, dict)
                and "errors" not in self._current_data.keys()
            ):
    ...
    def get_parameters_available(
        self, pop_type: str, parameter_type: str
    ) -> Dict[str, str]:
        """
        ...
        if self._logger:
            print(
                f"Unable to iterate over NoneType for {pop_type = } and {parameter_type = }"
            )
    ...
    def set_areas_by_pop_types(self) -> None:
        ...
        for pt in self._valid_pop_types:
            self._area_dict[pt] = self.get_parameters_available(
                pt, "area-types"
            )
    ...
    def set_dims_by_pop_types(self) -> None:
        ...
        for pt in self._valid_pop_types:
            self._dims_dict[pt] = self.get_parameters_available(
                pt, "dimensions"
            )
